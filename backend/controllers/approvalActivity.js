...
const { notifyDocumentApprovalStep, notifyDocumentDecline } = require("../services/emailService");
...
const getSubmittedDocs = async (req, res) => {
	try {
      ...
      ORDER BY request_documents.id DESC;`;
...
const getPendingDocs = async (req, res) => {
	try {
		const rawUserId = req.body?.userId || req.params?.userId || req.query?.userId;
		const rawRole = req.body?.role || req.params?.role || req.query?.role;

		const userId = rawUserId;
		let role = Array.isArray(rawRole) ? rawRole : [String(rawRole || "approver").toLowerCase()];
...
const approveDoc = async (req, res) => {
  try {
        console.log("trying to", req.body.data);
        ...
        if (isApproverRequired !== 1) {
          if (countRequiredApprovers > 0 && countApprovedApprovers < countRequiredApprovers) {
            return res.status(403).json({
              message: "Mandatory approvers must approve this document before optional approvers can act.",
              code: "403"
            });
          }
          if (quorum - countRequiredApprovers <= 0) {
            return res.status(403).json({
              message: "Mandatory approvers satisfy the total quorum for this stage. Optional approvers cannot act.",
              code: "403"
            });
          }
        }
...
const makeTransaction = async (req, res) => {
  try {
        ...
        console.log("trying to make transaction with", req.body.data);
  ...
  try {
    ...
    pool.getConnection((err, connection) => {
      ...
      connection.query(getDocQuery, [docId], (err, docResults) => {
        ...
        // Check approver eligibility for current stage
        ...
        connection.query(checkEligibilityQuery, [doctypeId, currentApprovalStage, userId, doctypeId, currentApprovalStage, userId, doctypeId, currentApprovalStage, doctypeId, currentApprovalStage, docId, currentApprovalStage], (eErr, eResults) => {
          ...
          if (!isUserMandatory) {
            if (mandatoryCount > 0 && approvedCount < mandatoryCount) {
              connection.release();
              return res.status(403).json({ message: "Mandatory approvers must approve this document before optional approvers can act.", code: "403" });
            }
            if (quorum - mandatoryCount <= 0) {
              connection.release();
              return res.status(403).json({ message: "Mandatory approvers satisfy the total quorum for this stage. Optional approvers cannot act.", code: "403" });
            }
          }
