...
const getDoctypes = async (req, res) => {
	try {
		const query = `SELECT c.id,c.description,c.trans_type,c.expense_code,c.account_number,c.account_desc,c.status,code_creations.code as code FROM code_creation_details c JOIN code_creations ON code_creations.id = 2 where c.code_id =2 ORDER BY c.id DESC;`;
	  // const query = `
...
const addDoctype = async (req, res) => {
	try {
		const { description, status, trans_type, expense_code,account_number, posted_by,account_desc } = req.body;


		console.log("Adding document type:", req.body);
		// Required data
		const dataEntry = [
...
const updateDoctype = async (req, res) => {
	try {
		const { description, trans_type, status, expense_code, id, posted_by,account_number,account_desc } = req.body;
